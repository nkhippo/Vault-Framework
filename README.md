...
tags:
  ...
  - landing
type: project_readme
updated: 2026-07-18T21:46:16+09:00
---
...
- **GitHub アカウント**(無料プラン可、Private リポジトリ無制限)
- **Cloudflare アカウント**(Workers 無料枠で運用可能、~1000 req/day 程度なら 0 円。カード登録は初期不要)
- **Claude アカウント + Code execution 有効化**:
  - 継続運用するなら **Pro / Max 以上を推奨**(月額 $20 〜、Free でも技術的には動作するが usage 上限で日常運用は厳しい)
  - **Skills 機能の前提として Settings > Capabilities で「Code execution and file creation」を有効化する必要がある**
  - Free / Pro / Max / Team / Enterprise のプラン別詳細は 2026-07 時点の公式ドキュメント(support.claude.com)で最終確認を推奨
...
5. Claude が前提の確認から順に対話でガイドします(所要 **初心者 4〜6 時間、経験者 2〜3 時間**、途中中断可)
...
初回セットアップ(Phase 1〜7)が完了したら、日常運用ガイドを参照:

- [`docs/ja/user-guide.md`](docs/ja/user-guide.md) — Vault-Framework の使い方全体(基本操作、Chat 引き継ぎ、プロジェクト管理、メンテナンス、トラブル対応 等)
...
| `docs/ja/user-guide.md` | **導入後の日常運用ガイド**(handoff / backlog / メンテ / トラブル 等) |
| `docs/ja/maintainer-guide.md` | **Framework 開発者・fork 保守者向け upstream 運用ガイド**(PII マスキング、影響ファイルマッピング、Cursor 指示書テンプレ、保護すべき既存構造 等) |
