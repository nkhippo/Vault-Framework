...
keywords:
  ...
  - claude-projects
status: published
summary: Claude Projects の Vault プロジェクト用 Instructions の汎用テンプレート(v1.6 相当)。SETUP.md 検知およびキックオフ検知による初期認識合わせセッション(Phase 7)、ケース 1〜5(特定プロジェクト / 執筆 / 汎用ナレッジ / 日記 / 意図不明)、特定プロジェクト相談時の総則、profile.md 参照、operations + applies_common 併読、guides 一括読み込み、日記・50_self 参照厳格化を定義する。
tags:
  - framework
  - vault-templates
  - meta
...
Phase 7 モードは以下のいずれかで発動する:

1. **SETUP.md 検知**(自動、最優先): Skill が `00_meta/SETUP.md` の存在を検知した場合、adopter からのキックオフ文言なしに Phase 7 モードへ入る。adopter が Vault-Framework 初期化直後の状態は基本的にこれに該当する
2. **キックオフ文言**(明示): 下記のキックオフ文言のいずれかを検知した場合
...
- **v1.6**: SETUP.md 検知による Phase 7 モード自動発動を明示化。Skill 側 Phase 0.0 と連動
