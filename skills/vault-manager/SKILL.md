...
## Chat 保存 + 残タスク統合フロー(Phase 1e 以降)

詳細規約は Vault-Framework `docs/ja/backlog/chat-save-with-residue-workflow.md` を参照。要点のみ記載。
...
**単純 chat 保存(「Vault に保存して」のみ)は従来通り**、統合フロー起動しない。判断困難時は Naoya に「残タスク起票も行いますか?」と 1 回確認。

### 実行フロー

1. **Chat 保存**: 既存の chat_log 保存 flow(Phase 1c 系)を実行、chat_log id 保持
2. **残 item 抽出**: Chat 全体を review、以下カテゴリを抽出
   - 未決事項(「後で検討」明示等)
   - 次アクション(未実行)
   - Blocker(依存関係)
   - 明示要求(Chat 内で「タスク化して」があったが未起票のもの)
   - 仕掛かり(結論・完了に至らなかった論点)
3. **候補リスト提示**: 表形式で | # | 種別 | 内容 | 推定 kind | 推定 project | を Naoya に提示
4. **Naoya 承認 gate**: 一括(全起票/全 skip)、個別選択、修正付き承認から選択
5. **Backlog 起票**: 承認 item ごとに Phase 1d PR-A の起票 flow を実行
   - Front Matter に `source_chat_log_id: <chat_log id>` を必ず追加
   - Body History に「chat_log <id> から自動抽出」を記録
6. **chat_log 逆リンク**: 起票発生時、chat_log Front Matter に `spawned_backlog_ids: [...]` を append
7. **完了報告**: chat_log id/path、起票 item 一覧、skip 数

### 抽出候補ゼロの場合

Chat が短い or 抽出対象なし → chat_log 保存のみで完了、backlog 化 step skip、報告に「残 item なし」明記。

### 判断困難な候補

- Kind 判定曖昧 → Naoya に確認
- Project 判定曖昧 → Naoya に確認 or `_life` fallback

### 相互リンクのスキーマ

- Backlog item: `source_chat_log_id`(Phase 1a schema、Phase 1e で必須付与)
- chat_log: `spawned_backlog_ids: [...]`(新規、optional、起票時に追加)
...
候補リスト → Naoya action → 実施。自動起票禁止(既存 Skill 原則継続)。

### 既存 workflow との関係

- 単純 chat 保存(「Vault に保存して」)、明示 backlog 起票(「これタスク化して」)、参照(「今仕掛かりは?」)は変更なし、独立動作
- 統合フローは既存 flow の合成として実現
