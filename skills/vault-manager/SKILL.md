...
- vault リポジトリ: `<your-account>/Vault`
...
- `create_issue(owner, repo, title, body, labels?, assignees?)` - Issue 起票(MVP: owner=<your-account> に固定)
...
4. 任意フィールド(mood, energy, weather)は あなた(導入者) が発話で言及した場合のみ埋める
...
- **あなた(導入者) が明示的に「日記を読み返して」「先週の日記を振り返って」等と指示しない限り、絶対に参照しない**
...
- 実装や仕様の技術的な一般論
- あなた(導入者) 自身の個人情報が必要ない Web 検索タスク
...
- あなた(導入者) が「更新した」と明示した場合のみ再取得を検討
...
一度読んだファイルは、同一 Chat 内では再度 MCP 経由で取得しない。Claude の context にすでに載っている前提で参照する。ファイル内容が更新された可能性がある場合(あなた(導入者) が「更新した」と明示した場合等)のみ再取得する。
...
### chat_log 保存 flow(Phase 1g 以降、即時実行)

**Primary trigger phrase**: 「会話の内容を Vault に保存して」

**Secondary(compat)**: 「Vault に保存して」、「Obsidian に保存して」、「保存して」(chat_log 保存意図が context から明確な場合)

**実行フロー(案提示なし、承認 gate なし)**:

1. Trigger 検知 → Chat context から chat_log の主題を判定
2. Skill が自律決定:
   - Path: `10_chat_logs/YYYY/MM/YYYY-MM-DD_<slug>.md`(または特定プロジェクトなら `30_projects/<Repo>/logs/YYYY/MM/`)
   - Title: 主題を短く表現
   - Tags: `[chat_log, ...]` + 主題 tag
   - Summary: Chat 内容の 1-2 行要約
   - ID: `kn-<YYYY-MM-DD>-<4hex>`(collision check)
3. Body 構築(Summary/Context/主要議論/Related PRs/History)
4. `create_note` を即実行
5. id / path を報告

Trigger 発話を承認扱いとし、案提示は行わない。あなた(導入者) は事後に `update_note` / `delete_note` で修正・削除できる。

**特殊ケース**:

- 対象プロジェクト特定不能 → `10_chat_logs/` 配下を default
- 50_self/ 関連等、機微内容の可能性 → あなた(導入者) に確認
- あなた(導入者) が「案を見せてから保存」と明示 → 案提示 → 承認 → 実施へ切り替え
...
- テンプレートの構造を踏襲
- 日記の場合、あなた(導入者) の発話をそのまま構造化して記入。要約しない
...
- 通常保存(上記 chat_log 即時保存 flow と Phase 1g の候補抽出 / handoff flow を除く): ユーザーに確認する
...
判断困難時は あなた(導入者) に確認し、`mt-` を fallback として提案する。
...
4. 3 回とも衝突したら あなた(導入者) にエラー報告(実質ほぼ発生しない)
...
3. 状態乖離検出:
   ...
   - Issue closed & not merged & Vault open → あなた(導入者) 確認
   - `updated` 2 週間以上 → `stalled` tag 提案
4. あなた(導入者) 承認 → `update_note` で Front Matter 更新 + H2 History 追記
...
4. あなた(導入者) 要求で `derived_from_id`, `related_ids`, `cursor_instruction_id`, `github_issue` を辿る

### 作業混ざり防止規約遵守

- 対象プロジェクト外の GitHub コネクタは能動使用しない
- 全体棚卸しは あなた(導入者) の明示指示があった時のみ許容、コネクタ切り替えを透明に伝える
- 50_self/ は明示指示なく参照しない

### あなた(導入者) 承認 gate(必須)

Backlog item の state/tag/FM 変更は **必ず** あなた(導入者) 承認を経る。無断更新禁止(既存 Skill 原則の継続)。
...
複数操作を含意する phrase は単独判断で処理せず、「複合 phrase への応答(Phase 1g 以降)」に従って あなた(導入者) に実行コマンドを確認する。

### 起票 flow の呼び出し元による gate 分岐(Phase 1g 以降)

- **候補抽出コマンド Step 2 からの内部呼び出し**: 保存 trigger 発話を承認扱いとし、gate skip で即時起票(ADR-0024)
- **あなた(導入者) の直接明示要求**(「これタスクとして起票して」等): 現状維持、案提示 → 承認 → 実施
- **Cursor 委譲 / GitHub Issue 起票 / open-questions 昇格**: 現状維持、案提示 → 承認 → 実施

### 起票 flow(共通)

1. 対象プロジェクト特定(Chat context または あなた(導入者) 確認)
2. **直接明示要求と gate 維持 flow のみ**起票案を あなた(導入者) に提示(候補抽出 Step 2 は skip)
3. **直接明示要求と gate 維持 flow のみ**あなた(導入者) 承認 gate(候補抽出 Step 2 は skip)
...
7. `create_note` で書き込み
8. あなた(導入者) に完了報告(id、path、kind、state、assignee)

### open-questions 昇格 flow(追加ステップ)

- 起票 flow の Step 2 で `derived_from_id` に open-questions ファイル id を含める
- 昇格後、あなた(導入者) に「open-questions の該当行を削除 / wikilink 置換 / 保持のどれ?」と選択させ、`update_note` で反映
...
- 起票する repo は あなた(導入者) 明示指示 または Chat context から特定
...
- 直接明示要求による新規起票、open-questions 削除・置換、Cursor 指示書作成、GitHub Issue 起票、backlog item の FM 変更

候補抽出 Step 2 は保存 trigger 発話を承認扱いとして即時起票する。Trigger なしの無断更新は禁止。

### 重複起票の回避

新規起票要求時、Claude は既存 backlog item がないか `search_by_keyword` で確認、既存があれば「既に起票済みです」と報告して重複回避。

## Backlog 候補抽出 workflow(Phase 1g 以降、2 step 即時実行)

詳細規約は Vault-Framework `docs/ja/backlog/candidate-extraction-workflow.md` を参照。要点のみ記載。
...
- Secondary: 「未決事項を backlog に候補リスト作って」、「候補リスト作って」、「残タスクを backlog 化して」

**Step 2(即保存)**:

- Primary: 「これを保存して」
- Secondary: 「全部保存して」、「起票して」(直前に Step 1 実行済みの context 時)
- Variant: 「1 と 3 だけ保存して」(部分)、「1 のタイトルを X に変えて保存」(修正)

### 実行フロー

**Step 1**(承認 gate なし、trigger 発話で抽出 + 提示):

1. Chat 全体から 5 カテゴリで抽出(未決事項/次アクション/Blocker/明示要求/仕掛かり)
2. 各 item の kind(task/issue)を Skill が推定
3. 各 item の project を Skill が推定(不明なら `_life` fallback)
4. 候補リストを表形式で提示: # / 種別 / 内容 / 推定 kind / 推定 project

**Step 2**(承認 gate なし、保存 trigger 発話で即実行):

5. 「これを保存して」等を検知
6. 全件または指定 subset を Phase 1d PR-A 起票 flow へ内部連携(gate skip)
7. 各 item の tags を選定し、`create_note` を逐次実行
8. chat_log 保存済みなら `source_chat_log_id` と `spawned_backlog_ids` を即時反映
9. 起票 item 一覧を報告

### Step 1 と Step 2 の間

- 全件 / 部分 / 修正保存、抽出やり直しを指示可能
- 別の話題へ移る → 自然に中断し、次の Step 1 trigger で再起動

### エラー処理

- 候補ゼロ → 「残 item なし」報告、起票なし
- MCP 失敗 → 中断ルール適用

## Handoff 保存 workflow(Phase 1g 以降、即時実行)

詳細規約は Vault-Framework `docs/ja/backlog/handoff-workflow.md` を参照。要点のみ記載。

### トリガー phrase

- Primary: 「別のチャットに引き継げるように Vault に保存して」
- Secondary: 「引き継ぎできるようにしてください」、「引き継ぎ用に保存」、「handoff を作って」、「別 Chat で続きから作業したい」

### 実行フロー(案提示なし、承認 gate なし)

1. Trigger 検知 → Chat context から対象プロジェクト判定
2. Skill が path/title/tags/summary/id を自律決定
3. Body 構築(Summary/Current work/Next steps/Open questions/Blockers/Related PRs/Related backlog items/Context)
4. `source_chat_log_id` と `related_backlog_ids` を該当時に付与
5. `create_note` で `30_projects/<Repo>/handoff/YYYY-MM-DD_<slug>.md` を即時作成
6. 必要に応じて `handoff/current-state.md` を更新
7. id / path / project を報告
8. **再会テンプレを Chat に出力**(下記必須)

### 履歴蓄積型

過去の `handoff/current-state.md` 上書き型からの移行、既存ファイルは保持。新規は timestamp 付き。

### 相互リンク

- handoff → chat_log: `source_chat_log_id`
- handoff → backlog: `related_backlog_ids`
- 逆方向は強制しない(handoff は時点スナップショット)

### 事後修正と特殊ケース

- あなた(導入者) は `update_note` / `delete_note` で事後修正可能
- 対象プロジェクト特定不能 → あなた(導入者) に確認
- 「案を見せてから保存」と明示 → 案提示 → 承認 → 実施へ切り替え

### handoff 保存後の再会テンプレ出力(必須)

handoff ファイル保存と `current-state.md` 更新が完了したら、adopter が新規 Chat で continuation するためのテンプレを Chat に出力する。

**出力フォーマット**:

必ずコードブロックで囲んで出力する(adopter がコピペしやすいように)。
...
「このメッセージと一緒に添付するもの」項目には、**MCP 経由で参照できないもの**を列挙する。具体的には:

- 前回 Chat 内で adopter がアップロードしたファイル(PDF / 画像 / スクリーンショット / CSV / コード等)
- 外部 URL のスクリーンショット(例: Cloudflare Dashboard の画面)
- 他リポジトリのコード(Vault リポジトリ外)
- Terminal 出力・エラーメッセージ

該当なしなら「特になし」と書く。判断に迷う場合は adopter に「新規 Chat で必要になりそうなファイルはありますか?」と確認して列挙する。

**出力後の後処理**:

- テンプレ出力後、adopter に「このテンプレを新規 Chat にコピペして、必要なファイルを添付してください」と案内
- adopter が新規 Chat を開いた後の対応は、そちら側の Claude が担当(このセッションでは終了)

## 複合 phrase への応答(Phase 1g 以降)

3 primary trigger のどれにも該当しない曖昧 phrase(例:「ここまでの会話を Vault に保存」)には、次を提示:

1. 会話の内容を Vault に保存(chat_log)
2. 会話内のタスクを Vault に保存(backlog 候補抽出)
3. 別のチャットに引き継げるように Vault に保存(handoff)

あなた(導入者) の選択で該当コマンドを順次実行。単独判断で解釈しない。
...
6. **あなた(導入者) に確認**: タイトル・本文・labels を提示、承認を得る
7. **`create_issue` を呼ぶ**: 承認後に MCP 経由で Issue 起票。使用する GitHub MCP コネクタは **プロジェクトが指定するもののみ**(作業混ざり防止、他プロジェクトのコネクタは絶対に呼ばない)
8. **報告**: Issue URL を あなた(導入者) に伝える

### MVP 制約

- **owner=<your-account> に固定**: MCP サーバ側で他 owner は 403 で拒否
- **PAT に Issues R/W が必要**: 未付与時は 403、あなた(導入者) に PAT 更新を促す

### Issue コメント追加

- Framework での議論の続きを既存 Issue にコメントとして追加する場合、`add_issue_comment` を使用
- コメントも あなた(導入者) の承認を得てから投稿
...
Cursor に以下を依頼します:
1. 対象アプリの複数ファイル修正
2. 修正後、あなた(導入者) の承認を得て Issue を close(または追加コメント)
```
...
- センシティブ情報(あなた(導入者) の個人的トピック)を含むファイルは Front Matter で `sensitive: true` を必ず設定
- **`sensitive: true` のファイルは、他コンテキストでの引用・要約を絶対に行わない**
- MCP コネクタ接続失敗時は本文書の該当セクションを厳守
- **v1.2 追加**: `create_issue` / `add_issue_comment` は あなた(導入者) の承認なしに実行しない(明示的な承認発話を受けてから)
...
- **`20_notes/guides/writing_examples.md`**(v1.4 追加、あなた(導入者) の指摘ペアの成長する集合)
...
- **v1.4**(2026-07-16):
  ...
  - 背景: 執筆ガイド未読のまま例集更新をスキップする不遵守が発生しうるため、guides 一括取得を正式化
- **v1.5**(2026-07-17):
...
- **v1.6**(2026-07-17):
  ...
  - GitHub Issue 状態確認と Vault backlog 同期提案(あなた(導入者) 承認 gate)
...
- **v1.7**(2026-07-17):
  ...
  - 案提示 → あなた(導入者) 承認 gate → 実施の 2 段階必須、重複起票回避
  - description に backlog 保存トリガー phrase を追加
  - 停滞検出は Phase 1d PR-B(vault-maintainer)の対象
- **v1.8**(2026-07-17):
  - Phase 1e: Chat 保存 + 残タスク統合フロー(引き継ぎ時の chat_log 保存 + 抽出 + 承認 gate + 一括起票)を追加
  - chat_log ↔ backlog 相互リンク(`source_chat_log_id` + `spawned_backlog_ids`)
  - description は unchanged、統合フローは body のみで担保
- **v1.9**(2026-07-17):
  - Phase 1e 統合フローを廃止し、chat_log 保存 / backlog 候補抽出 / handoff 保存の 3 独立コマンドに再構成
  - 複合 phrase は あなた(導入者) 確認後に任意の組合せを順次実行
  - description は unchanged
- **v1.10**(2026-07-17):
  - 3 独立コマンドに明示 primary trigger phrase を導入
  - chat_log / handoff は trigger 後に即時保存、候補抽出は抽出 → 保存 trigger の 2 step 即時実行
  - Cursor 委譲 / Issue 起票 / open-questions 昇格 / 直接明示起票の承認 gate は維持
  - description は unchanged
