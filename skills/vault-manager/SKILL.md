...
複数操作を含意する phrase は単独判断で処理せず、「複合 phrase への応答(Phase 1f 以降)」に従って Naoya に実行コマンドを確認する。
...
## Backlog 候補抽出 workflow(Phase 1f 以降)
...
**chat_log 保存(「Vault に保存して」)や handoff 作成(「引き継ぎできるようにしてください」)は本 workflow の対象外**。独立コマンドとして処理。

### 実行フロー

1. Chat 全体から抽出(未決事項/次アクション/Blocker/明示要求/仕掛かり の 5 カテゴリ)
2. 候補リスト提示(表形式: # / 種別 / 内容 / 推定 kind / 推定 project)
3. **Naoya 承認 gate**(一括/個別/修正付き/追加)
4. Phase 1d PR-A の起票 flow を再利用、承認 item を起票
5. chat_log 保存済みの場合 `source_chat_log_id` で紐付け、chat_log 側に `spawned_backlog_ids` を append 提案(Naoya 承認)
6. 完了報告(起票 N 件、skip M 件)
...
## Handoff 保存 workflow(Phase 1f 以降)
...
1. 対象プロジェクト特定(不明なら Naoya に確認)
2. Handoff 内容案を構築(Summary/Current work/Next steps/Open questions/Blockers/Related PRs/Related backlog items/Context)
3. Front Matter 構築案(type: handoff、project、tags: [handoff, chat, ...]、source_chat_log_id 済みなら含む、related_backlog_ids あれば含む)
4. **Naoya 承認 gate**
5. `create_note` で `30_projects/<Repo>/handoff/YYYY-MM-DD_<slug>.md` 作成(履歴蓄積型)
6. 完了報告(path、id、project)
...
## 複合 phrase への応答(Phase 1f 以降)

「ここまでの会話を Vault に保存」等、複数コマンドを含意する可能性がある発話を検知した時:

- Skill は 3 独立コマンドのどれか(または複数の順次実行)を Naoya に確認して分岐
- 例: 「chat_log 保存 / backlog 候補抽出 / handoff 作成 のどれ、または複数実行しますか?」
- Naoya の明示指示で任意の組合せを順次実行

複合コマンドを Skill が単独判断で解釈することは禁止。必ず Naoya に確認。
