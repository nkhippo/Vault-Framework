...
Trigger 発話を承認扱いとし、案提示は行わない。Naoya は事後に `update_note` / `delete_note` で修正・削除できる。

**特殊ケース**:

- 対象プロジェクト特定不能 → `10_chat_logs/` 配下を default
- 50_self/ 関連等、機微内容の可能性 → Naoya に確認
- Naoya が「案を見せてから保存」と明示 → 案提示 → 承認 → 実施へ切り替え
...
複数操作を含意する phrase は単独判断で処理せず、「複合 phrase への応答(Phase 1g 以降)」に従って Naoya に実行コマンドを確認する。

### 起票 flow の呼び出し元による gate 分岐(Phase 1g 以降)

- **候補抽出コマンド Step 2 からの内部呼び出し**: 保存 trigger 発話を承認扱いとし、gate skip で即時起票(ADR-0024)
- **Naoya の直接明示要求**(「これタスクとして起票して」等): 現状維持、案提示 → 承認 → 実施
- **Cursor 委譲 / GitHub Issue 起票 / open-questions 昇格**: 現状維持、案提示 → 承認 → 実施

### 起票 flow(共通)

1. 対象プロジェクト特定(Chat context または Naoya 確認)
2. **直接明示要求と gate 維持 flow のみ**起票案を Naoya に提示(候補抽出 Step 2 は skip)
3. **直接明示要求と gate 維持 flow のみ**Naoya 承認 gate(候補抽出 Step 2 は skip)
...
- Naoya は `update_note` / `delete_note` で事後修正可能
...
Naoya の選択で該当コマンドを順次実行。単独判断で解釈しない。
...
- **v1.9**(2026-07-17):
  ...
  - 複合 phrase は Naoya 確認後に任意の組合せを順次実行
