...
- **あなた(導入者) のアップロード運用**: ローカルの該当フォルダを zip 化 → Claude Settings > Skills にアップロード

## 現行バージョン

- **バージョン**: v1.4
- **確定日**: 2026-07-16
- **主要な追加**: note 執筆時の `20_notes/guides/` 4 ファイル一括取得を Level 1 トリガーとして正式化
...
Vault-MCP の `create_note` / `update_note` は仕様上、`updated` フィールドを自動付与する。これは Claude Skills では無視される想定だが、もしアップロード時にエラーが出た場合は ローカルディレクトリ 上で手動で `updated` 行を削除してから zip 化する。
...
- follow-up タスク: `id-ref-removed` の「直近のアクション」参照

## zip 化と upload の手順

あなた(導入者) が Skill を差し替える時の標準手順:

1. ローカルディレクトリ で `Vault/30_projects/Vault-Framework/skills/vault-manager/` フォルダを Finder で選択
...
Vault-MCP が自動付与した `updated` フィールドが原因の可能性。ローカルで該当行を手動削除して再度 zip 化 → アップロード。
