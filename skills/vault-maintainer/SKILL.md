...
あなた(導入者) の個人 Vault リポジトリ(`<your-account>/Vault`)の**保守運用(Level 2〜4)と抽象生成**を担当する Skill。日常の保存・参照・Level 1 自動修正は担当せず、それらは `vault-manager` Skill に委ねる。
...
| Level 4 | 季節(3ヶ月毎) | あなた(導入者) 主導 + Cursor | 計画立案 + 指示書作成 |
...
2. 検出結果を あなた(導入者) に提示(修正候補の一覧)
3. あなた(導入者) の承認を得て、Cursor 用の週次バッチ指示書を作成
...
各 item に対する action は あなた(導入者) の明示指示を経る:
...
Stalled 候補が `github_issue` を持つ場合、対象プロジェクトの GitHub コネクタで Issue state 確認可(作業混ざり防止規約遵守)。全体週次メンテでは複数コネクタ切り替えを あなた(導入者) に透明に伝えつつ実施可。
...
2. あなた(導入者) と変更計画をすり合わせる(複数セッションに分けることも)
...
4. **あなた(導入者) のレビュー**: 草案を提示、内容の充実と相互リンクの確認
...
- 保守運用・抽象生成のいずれも、実行前に あなた(導入者) の承認を得る
- 一括変更は必ず Cursor 委譲 + backup ブランチ作成を指示書に含める
- `delete_note` を伴う操作(廃止ファイルの削除等)は必ず あなた(導入者) 確認
...
- **v1.0**(2026-07-14): 初版。当初「3ヶ月の運用データ蓄積後に着手」として保留されていたが、あなた(導入者) の判断で保留を解除して作成。保守運用 Level 2〜4 と抽象生成を担当。vault-manager との棲み分けを明確化。実作業は Cursor 委譲を前提とする設計。
- **v1.1**(2026-07-17):
  - Phase 1d PR-B: Backlog stalled detection を Level 2 週次メンテに追加
  - Default threshold 14 日、あなた(導入者) 承認 gate 必須
