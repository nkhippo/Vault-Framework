...
summary: vault-maintainer が Level 2 週次メンテナンスで実行する backlog stalled detection の詳細規約。停滞 item の検出、tag/state 提案、あなた(導入者) 承認 gate。
---
...
vault-maintainer Skill が週次(Level 2)で実行する backlog stalled detection の詳細規約。周期的に停滞 item を検出し、あなた(導入者) に action 判断を促す。

## 位置付け(vault-manager との違い)

- **vault-manager の棚卸し flow(Phase 1c 参照 workflow)**: on-demand、あなた(導入者) の「棚卸しして」に応答
- **vault-maintainer の stalled detection**: 週次(または あなた(導入者) の「週次メンテ」トリガー)、周期的な自動検出
...
- あなた(導入者) が特定プロジェクトを指定した場合はそれに絞る
- 例: 「IPASoundDrill の stalled 検出して」→ `30_projects/IPASoundDrill/backlog/` のみ

### Step 2: Threshold 決定

- Default: 14 日(2 週間)
- あなた(導入者) が「直近 1 週間動きがないもの」等と指定した場合は 7 日等に変更
- 明示指示なければ 14 日を採用
...
- **A. Stalled tag 未付与**: 初めて停滞を検出、あなた(導入者) に stalled tag 付与を提案
- **B. Stalled tag 既付与**: 前回 stalled 判定済み、あなた(導入者) に state 変更(abandoned)or 「進捗確認要」を提案
...
- 提案 1: `stalled` tag を付与、あなた(導入者) は次回まで追跡
- 提案 2: `state: abandoned` に、`abandoned_reason` を あなた(導入者) に確認
- 提案 3: 即時進捗確認(あなた(導入者) が別途対応)
...
### Step 7: あなた(導入者) 承認 gate

各 item に対して個別に あなた(導入者) の action を得る:
...
- 全体週次メンテでは複数コネクタ切り替えを許容(あなた(導入者) の週次メンテ指示 = 全プロジェクト対象の意思表示)
- Issue が既に closed & merged なのに Vault が open → stalled というより「同期漏れ」として `state: done` 提案

## 作業混ざり防止規約遵守

- 対象プロジェクト外の GitHub コネクタは あなた(導入者) 明示指示時のみ使用
...
Weekly maintenance を あなた(導入者) が起動した時のフロー例:
