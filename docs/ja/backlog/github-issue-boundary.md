...
3. Cursor 委譲対象と判定(Skill vault-manager の Cursor 委譲判定ルール、または あなた(導入者) 判断)

Cursor 委譲対象でない小さい task は昇格せず、あなた(導入者) が手動対応 or 直接 Issue 起票。
...
2. あなた(導入者) が「棚卸しして」or「これ完了してるはず」と Claude に伝える
...
5. あなた(導入者) 承認 → Claude が update_note で反映

### あなた(導入者) 手動完了時

`assignee: owner` のまま完了した場合、あなた(導入者) が Claude に完了を伝える → Vault を `done` に更新。
...
- Cursor 委譲判定に該当しない小さい修正(1 ファイル数行等)→ あなた(導入者) 手動対応 or 直接 Issue

これらは `github_issue` フィールドを持たず、`assignee: owner` のまま完了する。
...
- Vault backlog に対応 item が無い Issue → 「これ Vault に backfill しますか?」と あなた(導入者) に提案
