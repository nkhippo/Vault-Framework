---
created: 2026-07-17 05:10:00+09:00
status: accepted
summary: Phase 1e 統合フローを廃止し、chat_log 保存 / 候補抽出 / handoff の 3 独立コマンドに再構成する意思決定記録。実運用第
  1 号 feedback を受けた仕様変更。
supersedes:
- ADR-0022
tags:
- adr
...
type: knowledge
updated: 2026-07-17 05:10:00+09:00
---
...
- **自動抽出の過剰**: 8 候補中 4 件が skip(50% skip 率)、抽出精度が あなた(導入者) の判断負荷を増やしている
...
「ここまでの会話を Vault に保存」等の複合 phrase を検知した時、Skill は 3 独立コマンドのどれか(または複数の順次実行)を あなた(導入者) に確認して分岐:

- 「chat_log 保存だけですか?」→ コマンド 1
- 「backlog 候補抽出も含めますか?」→ コマンド 1 + 2
- 「handoff 作成もしますか?」→ コマンド 1 + 2 + 3(または任意の組合せ)

あなた(導入者) の明示指示で複数コマンドを順次実行することは可能。
...
- **意思表示明確**: あなた(導入者) がどの操作を求めているかコマンドレベルで明確
...
**却下理由**: 2 系統の実装は Skill 肥大化 + あなた(導入者) の認知負荷増、どちらを使うかで迷いが発生。
...
- Candidate extraction workflow: candidate-extraction-workflow
...
- ADR-0024(Phase 1g、2026-07-17): 3 独立コマンド全てを即時実行に変更し、明示 trigger phrase を導入。コマンド 2 のみ 2 step(抽出 → 保存 trigger)。
