...
Naoya が Chat で複数の相談を並行して行い、途中で「後で考えましょう」となる課題や、別 Chat に切り出したタスクが増えている。全体像が見えなくなりつつあり、次のような要求が生まれた:
...
- Assignee: `naoya | cursor`(必須、null 禁止)
