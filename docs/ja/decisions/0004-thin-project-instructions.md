...
- あなた(導入者) の編集容易性: vault 側は Obsidian で直接編集できるが、Instructions は Claude UI 経由の編集が必要
...
- Project Instructions には以下のみを記述:
  1. Project の目的(例: 「あなた(導入者) の個人 Vault 運用の中核となる Chat 集約先」)
...
- Instructions の更新頻度が最小(用途変更時のみ)、あなた(導入者) の運用負荷が低い
...
- Instructions を見た人(あなた(導入者) 以外の第三者)は「これだけで動くのか?」と疑問を持つ可能性
...
- Instructions の更新頻度が高くなり、あなた(導入者) の運用負荷増
