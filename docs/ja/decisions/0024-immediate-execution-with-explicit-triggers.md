...
Phase 1f 実運用直後の Naoya feedback:
...
各コマンドに Naoya 指定の primary trigger phrase を採用し、既存類似 phrase は secondary(compat)として残す:
...
3 primary trigger のいずれにも該当しない曖昧 phrase(例:「ここまでの会話を Vault に保存」)には、3 コマンドの選択肢を提示し、Naoya の選択で分岐する。

## Consequences

### Positive

- Naoya の実運用意図と整合し、判断負荷と案提示 turn を削減
...
**却下理由**: Naoya の明示 feedback と不一致。

### C. コマンド 1, 2 のみ撤廃、3 は維持

**却下理由**: Naoya の訂正で handoff も即時実行と明示された。
