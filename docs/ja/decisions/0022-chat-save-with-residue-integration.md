---
created: 2026-07-17 03:30:00+09:00
status: superseded
...
### あなた(導入者) 承認 gate(必須維持)

- 候補リスト提示 → あなた(導入者) action → 実施
...
- 抽出漏れは あなた(導入者) が承認 gate で追加可能、過剰抽出は skip で除外可能

### Negative

- Skill body サイズ増加(description は触らない、body のみ)
- Chat 抽出精度への依存(Claude の理解精度が抽出品質を決める)
- あなた(導入者) のレビュー負荷(候補数次第、多い場合は疲弊の可能性)
...
**却下理由**: 実運用で頻繁に発生するユースケースで摩耗する、Skill の目的(あなた(導入者) のユースケース最適化)と不一致。

### B. Chat 保存時に常時自動抽出(承認 gate なし)

**却下理由**: あなた(導入者) 承認原則違反、過剰起票の危険、既存 Skill 挙動と不一致。

### C. Trigger phrase を追加せず、対話内で誘導するのみ

**却下理由**: 発火が Skill 側にないため trigger 認識できず、あなた(導入者) が毎回誘導する必要がある。
...
ADR-0023 Phase 1f Three Independent Commands により置換された。Phase 1e の実運用資産は保持し、本 ADR の内容も履歴として残す。
