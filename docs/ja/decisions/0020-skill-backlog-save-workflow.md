---
created: 2026-07-17 02:30:00+09:00
status: accepted
...
### 起票案の 2 段階(案提示 → あなた(導入者) 承認 → 実施)

- 全ての新規起票、Front Matter 変更、外部書き込み(open-questions 更新、Cursor 指示書作成、GitHub Issue 起票)は **必ず あなた(導入者) 明示承認**を経る
...
- あなた(導入者) の元々の目的(Chat 発生の課題・タスクを Vault で管理)が達成

### Negative

- Skill サイズ増加(要点のみ記載、詳細は save-workflow.md 参照で肥大化最小)
- 起票案提示 → 承認 → 実施の 2 段階は あなた(導入者) の対話回数が増える(品質優先の代償)
...
- Save workflow doc: save-workflow
- Skill: `skills/vault-manager/SKILL.md`

## Amended by

- ADR-0024(Phase 1g、2026-07-17): 承認 gate の適用範囲を整理。候補抽出コマンドからの呼び出し時は gate skip、あなた(導入者) の直接明示要求は現状維持。
