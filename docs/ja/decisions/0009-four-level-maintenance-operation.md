...
- **タイミング**: あなた(導入者) の意向で任意タイミング(推奨: 月次〜四半期)
- **担当**: Claude 提案 + あなた(導入者) の判断 + Cursor 実施
- **内容**:
...
- 抽象生成は あなた(導入者) の意向で任意タイミングで実施したい(定期化になじまない)
