...
3. **「§16 保護すべき既存構造」を確認**(★ v1.5.0 で追加された critical step)
...
[汎用化の判断] ← ここで stop する場合もある(個人色が強すぎる等)             
   |                                                                        
   v                                                                        
[保護対象の確認] ← §16 に照らして、既存 critical 構造への影響を判定         
   |                                                                        
   v                                                                        
[staging に反映]                                                            
...
既存 SKILL の変更時は必ず現行内容を `get_file_content` で取得し、**変更前後の diff を Naoya に提示**してから `skill_note` で書き込む。Phase 0.0(SETUP.md 検知)や handoff 保存後の再会テンプレ出力 等の既存挙動を意図せず削除しないため。詳細は §16。
...
**重要な原則**:
- 過去の教訓(rsync --delete 禁止、FM 二層、iCloud pull、§16 保護対象保持 等)を必ず「継承」セクションに列挙
...
1. 変更対象ファイル(候補)を §16.2 の「特に注意すべき既存構造」リストに照らす
2. 保護対象が含まれる場合、変更計画に「**保護対象への影響**」節を必ず追加
3. 変更が保護対象を**削除または意味変更**する場合、§16.3 に従って **MAJOR bump 相当**として Naoya に報告し、影響範囲を確認
...
- **全セクション**:このファイル自体が Framework 開発の正典。§16 を含む全構造を維持
