...
- project_aliases.md の具体的なエントリ内容(あなた(導入者) の実運用データ、vault 側で管理)
...
3. **判定過程の透明性**: 「〈RepoName〉と判断しました」と一言添える(あなた(導入者) が誤りに気づきやすい)
...
- **通称**: あなた(導入者) が実際に会話で使う呼び方(命名の揺らぎを吸収)
...
以下のフォーマットで候補を提示し、あなた(導入者) の確認を得る:
...
複数プロジェクトが同じ通称を持つ場合(例: 同じ略称が複数プロジェクトで使われる):
...
- あなた(導入者) が「別のプロジェクトです」と訂正した場合、Skill は次のセッションでその情報を活用したい
- しかしプロジェクト間で永続的な学習は Skill レベルでは行わない(Chat 内でのみ有効)
- 頻繁に発生する誤解は、あなた(導入者) が project_aliases.md の通称欄を編集して恒久対応

## References

- **関連 ADR**: 
  - `id-ref-removed`(1 Project 統合、あいまい名解決フローの前提)
- **関連 spec**: 
  - `id-ref-removed`(Level 2 遷移の詳細)
  - `id-ref-removed`(project フィールドの管理)
