...
- あなた(導入者) が「vault を更新した」と明示した場合のみ再取得
...
- 例外: あなた(導入者) が「更新した」と明示した場合のみ再取得
...
- **関連 ADR**: 
  - `id-ref-removed`(Skill の役割)
  - `id-ref-removed`(プロンプト工夫 2 の対応)
- **関連 spec**: 
  - `id-ref-removed`(Level 2 での handoff 参照)
  - `id-ref-removed`(Level 2 遷移の判定)
