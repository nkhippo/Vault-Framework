...
- **JST 固定**: すべての日付は あなた(導入者) のタイムゾーン(JST = UTC+9)基準
...
- `30_projects/<your-project>/`(GitHub リポジトリ名と一致)
...
- あなた(導入者) が海外滞在中の場合も JST 基準(vault の一貫性を優先)
...
- **関連 ADR**: 
  - `id-ref-removed`(リポジトリ命名)
  - `id-ref-removed`(保存先判断)
- **関連 spec**: 
  - `id-ref-removed`(Front Matter の title フィールド)
  - `id-ref-removed`(handoff/ 配下のファイル名例外)
