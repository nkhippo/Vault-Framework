...
Vault-Framework の技術要件は「Skills 有効化」と「Custom Connector 追加(MCP 接続)」の 2 つ。2026-07 時点の公式 Help Center によれば、これらは Free を含む全プランで利用可能(Free は Custom Connector 1 個までの制限で、本 Framework は 1 個で足りる)。ただし Platform Docs では差異があるため、**最新の公式ドキュメント(support.claude.com)で最終確認**を推奨。

- **技術的には Free でも動作可能**
- **継続運用するなら Pro 以上を推奨**:Free の usage 上限(送受信メッセージ数、Skills / Connector 呼び出し数)だと、Vault 経由の対話がすぐに詰まる
- **Settings > Capabilities で「Code execution and file creation」有効化**が Skills の前提
...
**保存先の目安**:
- 汎用議論・特定プロジェクト無し → `10_chat_logs/YYYY/MM/YYYY-MM-DD_topic.md`
