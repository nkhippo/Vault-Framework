...
- 例外: 「日報」「業務記録」は通常 chat_log として扱う(判断に迷ったら あなた(導入者) 確認)
...
- Step 1 で 90_inbox/ にフォールバックする場合、あなた(導入者) に「判断つかなかったので inbox に置きます」と一言添える
...
- 対応:
  ...
  - **通常の場合**: あなた(導入者) に確認(`_v2` 付与 or 別スラグ提案)
...
該当 type のテンプレート(`00_meta/templates/<type>.md`)から取得。詳細は `id-ref-removed` 参照。
...
- 未登録なら Level 1 の自動修正で類似値に置き換え or あなた(導入者) に確認
...
あなた(導入者) の発話をそのまま構造化して記入。要約しない、Claude の解釈を加えない。
...
- 判定できなければ 90_inbox/ に保存し、あなた(導入者) に報告
...
- Chat 内で あなた(導入者) が「間違えた、別のプロジェクト」と訂正した場合、Step 1 に戻る

## References

- **関連 ADR**: 
  - `id-ref-removed`(保存先思想)
  - `id-ref-removed`(50_self/ の扱い)
  - `id-ref-removed`(接続失敗ルール)
- **関連 spec**: 
  - `id-ref-removed`(ファイル名の詳細)
  - `id-ref-removed`(Front Matter の詳細)
  - `id-ref-removed`(Level 遷移)
- **関連 guideline**: 
  - `id-ref-removed`(Principle 7: 3 秒ルール)
  - `id-ref-removed`(Operational Pattern 2: 保存指示の受け方)
