...
人間(あなた(導入者))は Obsidian 等のツールを通じてこのデータを閲覧・編集するが、日々の主要なやり取り(保存、検索、参照)は Claude を介して行われる。この「AI-first」の設計思想が、Front Matter スキーマ、命名規約、参照レベルシステム等、Framework 全体の設計を貫いている。
