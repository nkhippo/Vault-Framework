...
詳細な規則、スラグ生成ロジック、Skill 側の自動生成アルゴリズムは file-naming spec を参照。
...
命名規則とは別に、Front Matter の `type`、`status`、`tags`、`project` フィールドには統制語彙(controlled vocabulary)を用いる。これはファイル名やディレクトリ名の「命名」とは異なるレイヤーの統制であり、詳細は vocabulary-design spec を参照。

## 関連

- [ADR 0006: 命名スキームの確定](../decisions/0006-naming-vault-scheme.md)
- file-naming spec: ファイル名規約の詳細仕様
- vocabulary-design spec: 統制語彙の設計
