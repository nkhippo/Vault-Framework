...
- framework
status: draft
summary: Vault-MCP が提供する全ツールの API リファレンス。各ツールの目的・パラメータ・戻り値・使用例・注意事項を統一フォーマットで記述。現在は導入部
  + 3 ツールサンプルのみ。残り 15 ツールは v1.1.0 で埋める。
tags:
- framework
...
updated: 2026-07-18 14:48:22+09:00
---
...
- `id` / `aliases` / `created` / `updated` は Vault-MCP が自動付与(実装依存、詳細は `env-vars.md` の `AUTO_INJECT_*`)
...
```json
{
  ...
  "arguments": {
    ...
    "mode": "replace_body",
    "update_frontmatter": {
      "updated": "2026-07-18T15:30:00+09:00",
      "status": "wip"
    },
...
- `append` は末尾に単に追加するのみ、フロントマターの `updated` は自動更新される(実装依存)
- `replace_all` は全内容を投入するため、フロントマター行を `content` に含めること
- SKILL.md ファイルは `skill_note` を使う(auto-inject される `updated` の扱いが異なる)
